...
경로의 내용이 달라져 SHA가 바뀌면 새 제출로 처리합니다. 팀별 제출은 기본 30회까지만
번호를 발급하며, 한도를 넘긴 파일은 채점 queue에 넣지 않습니다. 채점이나 점수 POST의
성공 여부와 무관하게 제출 번호가 발급된 고유 checkpoint를 한 번으로 세며, 같은 SHA의
중복 파일은 횟수에 포함하지 않습니다. 동시에 보관하는 pending
